...
// Global trigger (delegated) — intercept only on wide screens
function shouldUseModal() {
  // Desktop + tablet landscape as “wide”
  const wide = window.matchMedia('(min-width: 769px)').matches;
  const landscapeTablet = window.matchMedia('(orientation: landscape)').matches && window.innerWidth >= 700;
  return wide || landscapeTablet;
}
...
document.addEventListener('click', (e) => {
  ...
  // Phones / small tablet-portrait: let the browser navigate to the page
  if (!shouldUseModal() || !isPlainLeftClick(e)) return;

  // Wide screens: open modal instead of navigating
  e.preventDefault();
  lastTrigger = trigger;
...
// ---- AJAX submit for any form that opts in with data-ajax="web3"
document.addEventListener('submit', async (e) => {
  const form = e.target.closest('form[data-ajax="web3"]');
  if (!form) return;
  ...
  try {
    ...
    // Web3Forms returns JSON even on 200; we don’t need to read it for success UI
    statusEl.textContent = 'Thanks! Your request has been sent.';
    form.reset();
  } catch (err) {
    console.warn('Web3Forms submit failed:', err);
    statusEl.textContent = 'Sorry—could not submit right now. Please email me instead.';
  } finally {
